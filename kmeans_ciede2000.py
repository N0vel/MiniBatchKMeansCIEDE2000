...
def pairwise_distances_ciede2000_argmin_min(X, Y, axis=1):
    if axis == 0:
        X, Y = Y, X
    dist_mat = ciede2000_distance_matrix(X, Y)
    indices, values = np.argmin(dist_mat, axis=axis), np.min(dist_mat, axis=axis)
    return indices, values

def cart2polar_2pi(x, y):
    """
    [skimage.color.colorconv._cart2polar_2pi]
    """
...
def ciede2000(lab1, lab2, kL=1, kC=1, kH=1):
    """
    [Based on skimage.color.deltaE_ciede2000]
    """
    if lab1.ndim == 1:
        lab1 = np.expand_dims(lab1, 0)
    if lab2.ndim == 1:
        lab2 = np.expand_dims(lab2, 0)
    ...
    scale = 1 + G
    C1, h1 = cart2polar_2pi(a1 * scale, b1)
    C2, h2 = cart2polar_2pi(a2 * scale, b2)
    Lbar = 0.5 * (L1 + L2)
    tmp = (Lbar - 50) ** 2
...
def ciede2000_distance_matrix(X, Y):
    if Y.ndim == 1:
        Y = np.expand_dims(Y, 0)
    if X.ndim == 1:
        X = np.expand_dims(X, 0)
    X = X[:, :, None]
    return ciede2000(X, Y)
...
def _k_init(X, n_clusters, x_squared_norms, random_state, n_local_trials=None):
    ...
    closest_dist_sq = ciede2000_distance_matrix(
    ...
    for c in range(1, n_clusters):
        ...
        distance_to_candidates = ciede2000_distance_matrix(
            X[candidate_ids], X)
...
def _labels_inertia_precompute_dense(X, sample_weight, x_squared_norms,
    ...
    labels, mindist = pairwise_distances_ciede2000_argmin_min(
        X=X, Y=centers)
...
class KMeans(BaseEstimator, ClusterMixin, TransformerMixin):
    ...
    def _transform(self, X):
        ...
        return ciede2000_distance_matrix(X, self.cluster_centers_)
...
class MiniBatchKMeansCIEDE2000(KMeans):
